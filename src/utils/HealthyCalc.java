...
public abstract class HealthyCalc {
  ...
  public static float calcularIMC(final float peso, float altura) {
    altura = altura / 100;
    if (peso <= 0 || altura <= 0) {
  ...
  public static float taxaMetabolicaBasal(Pessoa pessoa) {
    ...
    if (pessoa.getGenero() == 'M') {
      return taxaAtividade * (66 + 13.7f * pessoa.getPeso() + 5 * (pessoa.getAltura()) - 6.8f * pessoa.getIdade()); // O
    }

    if (pessoa.getGenero() == 'F') {
      return taxaAtividade * (655 + 9.6f * pessoa.getPeso() + 1.8f * (pessoa.getAltura()) - 4.7f * pessoa.getIdade());
    }
